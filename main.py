...
def make_parser():
    ...
    parser.add_argument(
        "--neofile", default=(DATA_ROOT / "neos.csv"), type=pathlib.Path, help="Path to CSV file of near-Earth objects."
    )
    parser.add_argument(
        "--cadfile",
    ...
    inspect = subparsers.add_parser("inspect", description="Inspect an NEO by primary designation or by name.")
    inspect.add_argument(
        "-v", "--verbose", action="store_true", help="Additionally, print all known close approaches of this NEO."
    )
    ...
    query = subparsers.add_parser(
        "query", description="Query for close approaches that " "match a collection of filters."
    )
    filters = query.add_argument_group(
        "Filters", description="Filter close approaches by their attributes " "or the attributes of their NEOs."
    )
    filters.add_argument(
        "-d",
        "--date",
        type=date_fromisoformat,
        help="Only return close approaches on the given date, " "in YYYY-MM-DD format (e.g. 2020-12-31).",
    )
    filters.add_argument(
        "-s",
        "--start-date",
        type=date_fromisoformat,
        help="Only return close approaches on or after the given date, " "in YYYY-MM-DD format (e.g. 2020-12-31).",
    )
    filters.add_argument(
        "-e",
        "--end-date",
        type=date_fromisoformat,
        help="Only return close approaches on or before the given date, " "in YYYY-MM-DD format (e.g. 2020-12-31).",
    )
    filters.add_argument(
        "--min-distance",
        dest="distance_min",
    ...
    filters.add_argument(
        ...
        default=None,
        action="store_true",
        help="If specified, only return close approaches of NEOs that " "are potentially hazardous.",
    )
    filters.add_argument(
        "--not-hazardous",
        dest="hazardous",
        default=None,
        action="store_false",
        help="If specified, only return close approaches of NEOs that " "are not potentially hazardous.",
    )
    query.add_argument(
        "-l",
        "--limit",
        type=int,
        help="The maximum number of matches to return. " "Defaults to 10 if no --outfile is given.",
    )
    query.add_argument(
        "-o",
        "--outfile",
        type=pathlib.Path,
        help="File in which to save structured results. " "If omitted, results are printed to standard output.",
    )

    repl = subparsers.add_parser(
        "interactive",
        description="Start an interactive command session " "to repeatedly run `interact` and `query` commands.",
    )
    repl.add_argument(
        "-a",
        "--aggressive",
    ...
    def precmd(self, line):
        ...
        if changed:
            ...
            if not self.aggressive:
                print("To include these changes, please exit and restart this interactive session.", file=sys.stderr)
            else:
